...
Errors water_switch_add(char port, uint8_t pin_number, int16_t *assigned_id) {
    ...
    waterSwitches.waterSwitch[id].port = port;
    waterSwitches.waterSwitch[id].pin_number = pin_number;
    taskEXIT_CRITICAL();
    *assigned_id = id;

    return fsu50a_init(port, pin_number);
}
...
Errors water_switch_is_flowing(uint16_t id, uint8_t *is_flowing) {
    ...
    GPIO_PinState pin_state = fsu50a_is_set(waterSwitches.waterSwitch[id].port, waterSwitches.waterSwitch[id].pin_number);
