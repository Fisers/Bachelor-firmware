...
Errors pressure_sensor_add(char port, uint8_t pin_number, SPI_HandleTypeDef hspi, int16_t *assigned_id) {
    if(pressureSensors.last_index+1 > MAX_PRESSURE_SENSORS) return OUT_OF_RANGE;

    Errors error = abp2150pgsa_init(port, pin_number, hspi);
    ...
    pressureSensors.pressureSensor[id].port = port;
    ...
    error = pressure_sensor_read_data(id);
    if(error != NO_ERROR) {
        // Error occured, remove sensor from the list, decrease index
        taskENTER_CRITICAL();
        pressure_sensor_remove(id);
        pressureSensors.last_index--;
        taskEXIT_CRITICAL();
        return error;
    };
...
Errors pressure_sensor_read_data(uint16_t id) {
    ...
    error = abp2150pgsa_start_measure(pressureSensor->port, pressureSensor->pin_number, pressureSensor->hspi);
    if (error != NO_ERROR) {
    ...
    while(is_busy && tries < MAX_TRIES) {
        ...
        error = abp2150pgsa_is_busy(pressureSensor->port, pressureSensor->pin_number, pressureSensor->hspi, &is_busy);
        if (error != NO_ERROR) {
            pressureSensor->pressure = 0;
            pressureSensor->temperature = 0;
            return error;
    ...
    error = abp2150pgsa_read_data(pressureSensor->port, pressureSensor->pin_number, pressureSensor->hspi, return_data);
    if (error != NO_ERROR) {
        pressureSensor->pressure = 0;
        pressureSensor->temperature = 0;
        return error;
